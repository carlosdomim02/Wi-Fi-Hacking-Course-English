...
echo "If not wlan0, wlan1, wlan2, wlan3, wlan4 stop services and change this names in hostapd.conf, launch.sh and stop.sh" 
sleep 2

# Get WLAN0 MAC address
MAC_WLAN0=$(ip link show wlan0 | awk '/link\/ether/ {print $2}')
...
# Init access point
echo "AP up"
docker-compose exec access-point-1 /ap/start.sh "$WLAN0"
sleep 2

# Init and connect clients
echo "WLAN clients up"
docker-compose exec client-1 /client/start.sh "$WLAN1" "$MAC_WLAN0"
docker-compose exec client-2 /client/start.sh "$WLAN2" "$MAC_WLAN0"
docker-compose exec client-3 /client/start.sh "$WLAN3" "$MAC_WLAN0"
